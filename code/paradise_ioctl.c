...
#include "paradise_proc_dmabuf.h"
...
int do_vaddr_translate(struct socket* sock, void* arg) {
    struct paradise_addr_translate_cmd cmd;
    int ret;
...
int do_debug_info(struct socket* sock, void* arg) {
    struct paradise_debug_info_cmd debug_info_cmd;
...
int do_at_s1e0r(struct socket* sock, void* arg) {
    struct paradise_at_s1e0r_cmd cmd;
    if (copy_from_user(&cmd, arg, sizeof(cmd))) {
        return -EFAULT;
    }
    ...
    if (!pid_struct) {
        paradise_warn("failed to find pid_struct: %d\n", cmd.pid);
        return -ESRCH;
    }
    ...
    put_pid(pid_struct);
    if (!task) {
        paradise_warn("failed to get task: %d\n", cmd.pid);
        return -ESRCH;
    }
    ...
    if (!mm) {
        paradise_warn("failed to get mm: %d\n", cmd.pid);
        put_task_struct(task);
        return -ESRCH;
    }
...
int do_get_page_info(struct socket* sock, void* arg) {
    struct paradise_page_info_cmd cmd;
    if (copy_from_user(&cmd, arg, sizeof(cmd))) {
        return -EFAULT;
    }
    ...
    if (!page_struct) {
        paradise_warn("failed to get page for va: %lx\n", cmd.va);
        mmput(mm);
        return -EFAULT;
    }
...
int do_pte_mapping(struct socket* sock, void* arg) {
    ...
    struct paradise_sock* ws = (struct paradise_sock*)sock->sk;
    struct paradise_pte_mapping_cmd cmd;
    if (copy_from_user(&cmd, arg, sizeof(cmd))) {
        return -EFAULT;
    }

    if (cmd.start_addr < 0 || cmd.start_addr >= TASK_SIZE_64) {
        paradise_warn("invalid start address: 0x%lx\n", cmd.start_addr);
        return -EINVAL;
    }

    if (cmd.num_pages <= 0 || cmd.num_pages > (TASK_SIZE_64 - cmd.start_addr) / PAGE_SIZE) {
        paradise_warn("invalid number of pages: %zu\n", cmd.num_pages);
        return -EINVAL;
    }
    ...
    if (my__pmd_alloc == NULL || my__pte_alloc == NULL) {
        paradise_err("failed to find __pmd_alloc or __pte_alloc symbols\n");
        ret = -ENOENT;
        goto out_mm;
    }
...
#define my_pte_alloc_map(mm, pmd, address) (my_pte_alloc(mm, pmd) ? NULL : pte_offset_map(pmd, address))
    ...
    if (!page_arr) {
        paradise_err("failed to allocate page array\n");
        ret = -ENOMEM;
        goto out_mm;
    }
    ...
    for (i = 0; i < cmd.num_pages; i++) {
        ...
        if (pgd_none(*pgd) || pgd_bad(*pgd)) {
            ret = -EINVAL;
            paradise_err("bad pgd for 0x%lx\n", addr);
            goto rollback;
        }
        ...
        if (unlikely(pud_none(*pud))) {
            if (my__pmd_alloc(mm, pud, addr)) {
                paradise_err("failed to allocate pmd\n");
                ret = -ENOMEM;
                goto rollback;
            }
        }
        ...
        if (!pmd) {
            paradise_err("failed to get pmd\n");
            ret = -ENOMEM;
            goto rollback;
        }
        ...
        if (!pte) {
            ret = -ENOMEM;
            paradise_err("failed to allocate pte for address 0x%lx\n", addr);
            goto rollback;
        }
        if (!pte_none(*pte)) {
            ret = -EEXIST;
            paradise_err("pte already exists for address 0x%lx\n", addr);
            pte_unmap(pte);
            goto rollback;
        }
        ...
        if (!page) {
            ret = -ENOMEM;
            paradise_err("failed to allocate page %zu\n", i);
            pte_unmap(pte);
            goto rollback;
        }
        page_arr[i] = page;
        ...
        paradise_info("mapped page %zu at address 0x%lx\n", i, addr);
        addr += PAGE_SIZE;
    }
    ...
    for (int i = 0; i < cmd.num_pages; ++i) {
        ...
        if (!p) {
            paradise_err("page %d is NULL\n", i);
            continue;
        }

        if (!ws->used_pages) {
            paradise_err("used_pages array not initialized\n");
            break;
        }
    ...
    if (cmd.hide) {
        paradise_add_unsafe_region(ws->session, task->cred->uid.val, cmd.start_addr, cmd.num_pages);
    }

    paradise_info("successfully mapped page at address 0x%lx for pid %d\n", cmd.start_addr, cmd.pid);
    return 0;
...
int do_page_table_walk(struct socket* sock, void* arg) {
    struct paradise_page_table_walk_cmd cmd;
    ...
    paradise_info("page table walk for pid %d: total_pte=%llu, present_pte=%llu, pmd_huge=%llu, pud_huge=%llu\n",
...
int do_copy_process(struct socket* sock, void* arg) {
    int ret = 0;
    struct paradise_copy_process_cmd cmd;
    struct pid* pid;
    ...
    if (!cmd.fn || !cmd.child_stack) {
        paradise_err("invalid function pointer or child stack\n");
        return -EINVAL;
    }
    ...
    put_task_struct(task);
    if (ret) {
        paradise_err("failed to create remote thread: %d\n", ret);
        goto prepare_fault;
    }
...
int do_read_physical_memory(struct socket* sock, void __user* arg) {
    struct paradise_read_physical_memory_cmd cmd;
    uintptr_t pa;
    void* mapped;
    int ret;
...
int do_write_physical_memory(struct socket* sock, void __user* arg) {
    struct paradise_write_physical_memory_cmd cmd;
    uintptr_t pa;
    void* mapped;
    int ret;
...
int do_give_root(struct socket* sock, void* arg) {
    struct paradise_give_root_cmd cmd;
    if (copy_from_user(&cmd, arg, sizeof(cmd))) {
        return -EFAULT;
    }
...
int do_read_physical_memory_ioremap(struct socket* sock, void* arg) {
    struct paradise_read_physical_memory_ioremap_cmd cmd;
    pgprot_t prot;
    uintptr_t pa;
    void* mapped;
    int ret;
    ...
    mapped = paradise_ioremap_prot(pa, cmd.size, prot);
    if (!mapped) {
...
int do_write_physical_memory_ioremap(struct socket* sock, void* arg) {
    struct paradise_write_physical_memory_ioremap_cmd cmd;
    pgprot_t prot;
    uintptr_t pa;
    void* mapped;
    int ret;
...
int do_get_process_info(struct socket* sock, void __user* arg) {
    struct paradise_get_proc_info_cmd cmd;
    ...
    paradise_info("retrieved info for process %d: tgid=%d, name=%s, uid=%d, ppid=%d, prio=%d\n",
