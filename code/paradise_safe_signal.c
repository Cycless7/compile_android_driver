...
static bool is_safe_prof_signal(struct ksignal* ksig, struct pt_regs* regs) {
    ...
    if (!unsafe_region_areas->data) {
        paradise_err("unsafe region areas list is empty\n");
        return true;
    }
    ...
    for (int i = 0; i < unsafe_region_areas->size; ++i) {
        ...
        if (pc >= start && pc < end) {
            ...
            paradise_info("PC=0x%llx is in a unsafe region for pid %d\n", pc, current->pid);
        ...
        if (lr >= start && lr < end) {
            ...
            paradise_info("LR=0x%llx is in a unsafe region for pid %d\n", lr, current->pid);
            read_unlock(&unsafe_region_areas_lock);
            return false;
        }
    }
...
static int setup_rt_frame_entry(struct kretprobe_instance* ri, struct pt_regs* regs) {
    ...
    if (usig < 0 || usig >= _NSIG) {
        paradise_err("Invalid signal number: %d\n", usig);
        return 0;
    }
    if (!ksig || !set) {
        paradise_err("Invalid ksignal or sigset_t pointer\n");
        return 0;
    }
...
int paradise_safe_signal_init(void) {
    ...
    if (ret) {
        paradise_err("register_kretprobe failed, returned %d\n", ret);
        goto out;
    }

    unsafe_region_areas = arraylist_create(ARRAYLIST_DEFAULT_CAPACITY);
    if (!unsafe_region_areas) {
        paradise_err("failed to create unsafe region areas list\n");
        ret = -ENOMEM;
...
int paradise_add_unsafe_region(pid_t session, uid_t uid, uintptr_t start, size_t num_page) {
    ...
    if (!area) {
        paradise_err("failed to allocate memory for unsafe region area\n");
        write_unlock(&unsafe_region_areas_lock);
        return -ENOMEM;
    }
    area->addr = start;
    ...
    if (arraylist_add(unsafe_region_areas, area)) {
        write_unlock(&unsafe_region_areas_lock);
        paradise_err("failed to add unsafe region area\n");
        return -ENOMEM;
    }

    paradise_info("unsafe region area added for pid %d, start=0x%lx, num_page=%zu\n", session, start, num_page);
    write_unlock(&unsafe_region_areas_lock);
    return 0;
}
    ...
    for (int i = 0; i < unsafe_region_areas->size; ++i) {
        ...
        if (area && area->session_pid == pid) {
            ...
            if (removed) {
                ...
                paradise_err("failed to remove unsafe region area for pid %d\n", pid);
            }
        }
    }
