...
pte_t* page_from_virt_user(struct mm_struct* mm, uintptr_t va) {
    ...
    if (pgd_none(*pgd) || pgd_bad(*pgd)) {
        paradise_warn("PGD entry for address 0x%lx not found or bad\n", va);
        goto out;
    }

    p4d = p4d_offset(pgd, va);
    if (p4d_none(*p4d) || p4d_bad(*p4d)) {
        paradise_warn("P4D entry for address 0x%lx not found or bad\n", va);
        goto out;
    }

    pud = pud_offset(p4d, va);
    if (pud_none(*pud) || pud_bad(*pud)) {
        paradise_warn("PUD entry for address 0x%lx not found or bad\n", va);
        goto out;
    }

    if (pud_leaf(*pud)) {
        paradise_debug("Address 0x%lx maps to a PUD-level huge page (leaf), no PTE exists\n", va);
        goto out;
    }

    pmd = pmd_offset(pud, va);
    if (pmd_none(*pmd) || pmd_bad(*pmd)) {
        paradise_warn("PMD entry for address 0x%lx not found or bad\n", va);
        goto out;
    }

    if (pmd_leaf(*pmd)) {
        paradise_debug("Address 0x%lx maps to a PMD-level huge page (leaf), no PTE exists\n", va);
        goto out;
    }

    ptep = pte_offset_kernel(pmd, va);
    if (!ptep) {
        paradise_warn("Failed to map PTE for address 0x%lx\n", va);
        goto out;
    }
out:
    MM_READ_UNLOCK(mm);
...
uintptr_t vaddr_to_phy_addr(struct mm_struct* mm, uintptr_t va) {
    if (!mm) {
        paradise_warn("mm_struct is NULL, cannot perform translation\n");
        return 0;
    }

    pte_t* ptep = page_from_virt_user(mm, va);
    if (!ptep) {
        paradise_err("failed to get PTE for virtual address 0x%lx\n", va);
        return 0;
    }

    if (!pte_present(*ptep)) {
        paradise_err("PTE not present for virtual address 0x%lx\n", va);
        return 0;
    }
...
#define paradise_phys_to_pfn(paddr) W_PHYS_PFN(paddr)
...
#endif
    return pfn_to_page(paradise_phys_to_pfn(vaddr_to_phy_addr(mm, va)));
}
...
void __iomem* paradise_ioremap_prot(uintptr_t phys_addr, size_t size, pgprot_t prot) {
    ...
    if (my_ioremap_page_range == NULL || my_free_vm_area == NULL) {
        ...
        if (my_ioremap_page_range == NULL || my_free_vm_area == NULL) {
            paradise_err("cannot find ioremap_page_range or free_vm_area\n");
            return NULL;
        }
    }
    ...
    if (my__get_vm_area_caller == NULL) {
        ...
        if (my__get_vm_area_caller == NULL) {
            paradise_err("cannot find __get_vm_area_caller\n");
            return NULL;
        }
    }
    ...
    if (my_get_vm_area_caller == NULL) {
        ...
        if (my_get_vm_area_caller == NULL) {
            paradise_err("cannot find get_vm_area_caller\n");
            return NULL;
        }
    }
    ...
    switch (wmt_type) {
        ...
        case WMT_NORMAL_TAGGED:
            ...
            *prot_out = __pgprot(PROT_NORMAL_TAGGED);
            return 0;
#else
            paradise_warn("PROT_NORMAL_TAGGED not defined on this kernel\n");
            return -EINVAL;
#endif
        ...
        case WMT_NORMAL_NC:
            ...
            *prot_out = __pgprot(PROT_NORMAL_NC);
            return 0;
#else
            paradise_warn("PROT_NORMAL_NC not defined on this kernel\n");
            return -EINVAL;
#endif
        ...
        case WMT_NORMAL_WT:
            ...
            *prot_out = __pgprot(PROT_NORMAL_WT);
            return 0;
#else
            paradise_warn("PROT_NORMAL_WT not defined on this kernel\n");
            return -EINVAL;
#endif
        ...
        case WMT_DEVICE_nGnRnE:
            ...
            *prot_out = __pgprot(PROT_DEVICE_nGnRnE);
            return 0;
#else
            paradise_warn("PROT_DEVICE_nGnRnE not defined on this kernel\n");
            return -EINVAL;
#endif
        ...
        case WMT_DEVICE_nGnRE:
            ...
            *prot_out = __pgprot(PROT_DEVICE_nGnRE);
            return 0;
#else
            paradise_warn("PROT_DEVICE_nGnRE not defined on this kernel\n");
            return -EINVAL;
#endif

        default:
            paradise_warn("invalid prot: %d\n", wmt_type);
            return -EINVAL;
    }
}