...
#include "paradise_page_walk.h"
#include "paradise_utils.h"

struct paradise_mapped_page_info {
    uintptr_t phys_addr; /* Physical page address (page-aligned) */
    void* mapped_ptr; /* Kernel virtual address from ioremap */
};

struct paradise_bindproc_private {
    pid_t pid;
    pgprot_t prot; /* Memory protection type (use WMT_*) */
    struct karray_list* mapped_pages; /* of struct paradise_mapped_page_info * */
    struct mutex lock; /* Protects mapped_pages access */
};

/* Helper: Find cached mapping for physical page */
static void* find_cached_mapping(struct paradise_bindproc_private* priv, uintptr_t phys_page) {
    size_t i;
    for (i = 0; i < priv->mapped_pages->size; i++) {
        struct paradise_mapped_page_info* info = arraylist_get(priv->mapped_pages, i);
...
static int add_cached_mapping(struct paradise_bindproc_private* priv, uintptr_t phys_page, void* mapped) {
    struct paradise_mapped_page_info* info = kmalloc(sizeof(*info), GFP_KERNEL);
    if (!info) {
        return -ENOMEM;
    }
...
#define PARADISE_BP_IOCTL_SET_MEMORY_PROT _IOWR('B', 1, int) /* arg: int (WMT_*) */
#define PARADISE_BP_IOCTL_READ_MEMORY _IOWR('B', 2, struct bp_read_memory_cmd)
#define PARADISE_BP_IOCTL_WRITE_MEMORY _IOWR('B', 3, struct bp_write_memory_cmd)

static long bindproc_ioctl(struct file* f, unsigned int cmd, unsigned long arg) {
    struct paradise_bindproc_private* private_data = f->private_data;
    int ret = 0;
    if (!private_data) {
        return -EINVAL;
    }
    ...
    switch (cmd) {
    case PARADISE_BP_IOCTL_SET_MEMORY_PROT:
        {
            int prot;
            pgprot_t new_prot;
            ...
            if(pgprot_val(new_prot) != pgprot_val(private_data->prot)) {
                ...
                for (i = 0; i < private_data->mapped_pages->size; i++) {
                    struct paradise_mapped_page_info* info = arraylist_get(private_data->mapped_pages, i);
                    if (info) {
                        if (info->mapped_ptr) {
            ...
            paradise_info("set memory prot to %d for pid %d\n", prot, private_data->pid);
            return ret;
        }
    case PARADISE_BP_IOCTL_READ_MEMORY:
        {
            ...
            if (cmd.size == 0 || cmd.size > (PAGE_SIZE * 16)) {
                paradise_err("invalid read size: %zu\n", cmd.size);
                return -EINVAL;
            }

            va = cmd.src_va;
            //paradise_info("bindproc_read: pid=%d, va=0x%lx, size=%zu\n", private_data->pid, va, cmd.size);
            ...
            while (total_read < cmd.size) {
                ...
                if (ret < 0) {
                    paradise_err("failed to translate VA 0x%lx: %d\n", va + total_read, ret);
                    goto out;
                }
                ...
                //paradise_info("Reading %zu bytes from VA 0x%lx (PA 0x%lx)\n", bytes_to_read, va + total_read, pa);
                //paradise_info("  Page start: 0x%lx, Offset: 0x%lx\n", page_start, offset);

                /* Check cache for existing mapping */
                mapped = find_cached_mapping(private_data, page_start);
                if (!mapped) {
                    /* Not cached, create new mapping */
                    mapped = paradise_ioremap_prot(page_start, PAGE_SIZE, private_data->prot);
                    if (!mapped) {
                        paradise_err("failed to ioremap physical address 0x%lx\n", page_start);
                        ret = -ENOMEM;
                        goto out;
                    }
                    ...
                    if (ret < 0) {
                        paradise_err("failed to cache mapping for PA 0x%lx\n", page_start);
                        iounmap(mapped);
                        goto out;
                    }
                }
                ...
                if (ret != 0) {
                    paradise_err("copy_to_user failed: %d bytes not copied\n", ret);
                    ret = -EFAULT;
                    goto out;
                }
    ...
    case PARADISE_BP_IOCTL_WRITE_MEMORY:
        {
            ...
            if (cmd.size == 0 || cmd.size > (PAGE_SIZE * 16)) {
                paradise_err("invalid write size: %zu\n", cmd.size);
                return -EINVAL;
            }
            ...
            while (total_written < cmd.size) {
                ...
                if (ret != 0) {
                    paradise_err("copy_from_user failed: %d bytes not copied\n", ret);
                    ret = -EFAULT;
                    goto out_write;
                }
    ...
    default:
        paradise_err("unknown ioctl cmd: 0x%x\n", cmd);
        return -ENOTTY;
    }
...
int do_bind_proc(struct socket* sock, void __user* arg) {
    struct paradise_bind_proc_cmd cmd;
    struct paradise_bindproc_private* private_data = NULL;
    ...
    if (!task) {
        paradise_err("failed to find task for pid: %d\n", cmd.pid);
        return -ESRCH;
    }
    ...
    if (!private_data) {
        paradise_err("failed to allocate memory for private_data\n");
        return -ENOMEM;
    }
    ...
    if (!private_data->mapped_pages) {
        paradise_err("failed to create mapped_pages arraylist\n");
        ret = -ENOMEM;
    ...
    if (fd < 0) {
        paradise_err("failed to get unused fd: %d\n", fd);
        ret = fd;
        goto err_free_private;
    }

    /* Create anonymous inode file */
    filp = anon_inode_getfile("[paradise_bindproc]", &bindproc_fops, private_data, O_RDWR | O_CLOEXEC);
    if (IS_ERR(filp)) {
        paradise_err("failed to create anon inode file: %ld\n", PTR_ERR(filp));
        ret = PTR_ERR(filp);
        goto err_put_fd;
    }
    ...
    if (copy_to_user(arg, &cmd, sizeof(cmd))) {
        paradise_err("failed to copy cmd back to user\n");
        ret = -EFAULT;
        goto err_fput;
    }
