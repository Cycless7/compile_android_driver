...
struct paradise_debug_info_cmd {
...
struct paradise_dma_buf_create_cmd {
    pid_t pid;
    uintptr_t va;
    size_t size;
    int fd;
...
struct paradise_pte_mapping_cmd {
    pid_t pid;
...
struct paradise_page_table_walk_cmd {
...
struct paradise_copy_process_cmd {
    pid_t pid;
    int (*__user fn)(void*);
...
struct paradise_get_proc_info_cmd {
...
#define PARADISE_IOCTL_ADDR_TRANSLATE _IOWR('W', 1, struct paradise_addr_translate_cmd)
/* IOCTL command for debugging information */
#define PARADISE_IOCTL_DEBUG_INFO _IOR('W', 2, struct paradise_debug_info_cmd)
/* * IOCTL command for va to phys translation */
#define PARADISE_IOCTL_AT_S1E0R _IOWR('W', 3, struct paradise_at_s1e0r_cmd)
/* IOCTL command for getting page information at a specific virtual address */
#define PARADISE_IOCTL_PAGE_INFO _IOWR('W', 4, struct paradise_page_info_cmd)
/* IOCTL command for creating a DMA buffer at a specific virtual address */
#define PARADISE_IOCTL_DMA_BUF_CREATE _IOWR('W', 5, struct paradise_dma_buf_create_cmd)
/* IOCTL command for getting PTE mapping information */
#define PARADISE_IOCTL_PTE_MAPPING _IOWR('W', 6, struct paradise_pte_mapping_cmd)
/* IOCTL command for page table walk */
#define PARADISE_IOCTL_PAGE_TABLE_WALK _IOWR('W', 7, struct paradise_page_table_walk_cmd)
/* IOCTL command for copying a process */
#define PARADISE_IOCTL_COPY_PROCESS _IOWR('W', 8, struct paradise_copy_process_cmd)
...
#define PARADISE_IOCTL_GIVE_ROOT _IOWR('W', 15, struct paradise_give_root_cmd)
...
#define PARADISE_IOCTL_BIND_PROC _IOWR('W', 18, struct paradise_bind_proc_cmd)
...
#define PARADISE_IOCTL_GET_PROC_INFO _IOWR('W', 20, struct paradise_get_proc_info_cmd)
...
static const struct ioctl_cmd_map {
    ...
    {.cmd = PARADISE_IOCTL_ADDR_TRANSLATE, .handler = do_vaddr_translate},
    {.cmd = PARADISE_IOCTL_DEBUG_INFO, .handler = do_debug_info},
    {.cmd = PARADISE_IOCTL_AT_S1E0R, .handler = do_at_s1e0r}, /* Reusing the same handler for AT VA */
    {.cmd = PARADISE_IOCTL_PAGE_INFO, .handler = do_get_page_info},
    {.cmd = PARADISE_IOCTL_DMA_BUF_CREATE, .handler = do_create_proc_dma_buf},
    {.cmd = PARADISE_IOCTL_PTE_MAPPING, .handler = do_pte_mapping},
    {.cmd = PARADISE_IOCTL_PAGE_TABLE_WALK, .handler = do_page_table_walk},
    {.cmd = PARADISE_IOCTL_COPY_PROCESS, .handler = do_copy_process},
    ...
    {.cmd = PARADISE_IOCTL_GIVE_ROOT, .handler = do_give_root},
    {.cmd = PARADISE_IOCTL_READ_MEMORY_IOREMAP, .handler = do_read_physical_memory_ioremap},
    {.cmd = PARADISE_IOCTL_WRITE_MEMORY_IOREMAP, .handler = do_write_physical_memory_ioremap},
    {.cmd = PARADISE_IOCTL_BIND_PROC, .handler = do_bind_proc},
    {.cmd = PARADISE_IOCTL_LIST_PROCESSES, .handler = do_list_processes},
    {.cmd = PARADISE_IOCTL_GET_PROC_INFO, .handler = do_get_process_info},
