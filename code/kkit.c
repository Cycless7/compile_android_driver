...
int paradise_flip_open(const char *filename, int flags, umode_t mode, struct file **f) {
    ...
    *f = filp_open(filename, flags, mode);
    ...
    if (reserve_flip_open == NULL) {
        reserve_flip_open = (struct file* (*)(const char *filename, int flags, umode_t mode))paradise_kallsyms_lookup_name("filp_open");
        if (reserve_flip_open == NULL) {
            return -1;
        }
    }
...
int paradise_flip_close(struct file **f, fl_owner_t id) {
    ...
    if (reserve_flip_close == NULL) {
        reserve_flip_close = (struct file* (*)(struct file **f, fl_owner_t id))paradise_kallsyms_lookup_name("filp_close");
        if (reserve_flip_close == NULL) {
            return -1;
        }
    }
...
unsigned long *paradise_find_syscall_table(void) {
    ...
    syscall_table = (unsigned long*)paradise_kallsyms_lookup_name("sys_call_table");
...
int mark_pid_root(pid_t pid) {
    ...
    if (task == NULL){
        printk(KERN_ERR "[paradise] Failed to get current task info.\n");
        return -1;
    }

    if (my_prepare_creds == NULL) {
        my_prepare_creds = (void *) paradise_kallsyms_lookup_name("prepare_creds");
        if (my_prepare_creds == NULL) {
            printk(KERN_ERR "[paradise] Failed to find prepare_creds\n");
            return -1;
        }
    }
    ...
    if (new_cred == NULL) {
        printk(KERN_ERR "[paradise] Failed to prepare new credentials\n");
        return -ENOMEM;
    }
    new_cred->uid = kuid;
    new_cred->gid = kgid;
...
static void foreach_process(void (*callback)(struct paradise_task_struct *)) {
    struct task_struct *task;
    struct paradise_task_struct paradise_task;
    int ret = 0;

    if (my_get_cmdline == NULL) {
        my_get_cmdline = (void *) paradise_kallsyms_lookup_name("get_cmdline");
    }
    ...
    for_each_process(task) {
        ...
        paradise_task = (struct paradise_task_struct) {
                .task = task,
                .cmdline_len = 0
        };

        memset(paradise_task.cmdline, 0, 256);
        if (my_get_cmdline != NULL) {
            ret = my_get_cmdline(task, paradise_task.cmdline, 256);
            if (ret < 0) {
                continue;
            }
            paradise_task.cmdline_len = ret;
        }

        callback(&paradise_task);
    }
    rcu_read_unlock();
}
...
pid_t find_process_by_name(const char *name) {
	...
	if (name_len == 0) {
		pr_err("[paradise] process name is empty\n");
		return -2;
	}
    ...
    for_each_process(task) {
        ...
        if (ret < 0) {
            ...
            pr_warn("[paradise] Failed to get cmdline for pid %d\n", task->pid);
