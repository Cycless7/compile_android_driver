...
int get_last_driver_slot(struct input_dev* dev) {
	...
	if(!dev) {
		pr_err("[paradise] wtf? dev is null\n");
		return -114;
	}
				 ...
				 unsigned int type, unsigned int code, int value)
{
	if(my_input_handle_event == NULL) {
		my_input_handle_event = (void (*)(struct input_dev *, unsigned int, unsigned int, int))paradise_kallsyms_lookup_name("input_handle_event");
	}

	if (!my_input_handle_event) {
		pr_err("[paradise] Holy fuck!Failed to find input_handle_event\n");
		return -1;
	}
...
struct input_dev* find_touch_device(void) {
	...
	input_dev_list = (struct list_head *)paradise_kallsyms_lookup_name("input_dev_list");
	input_mutex = (struct mutex *)paradise_kallsyms_lookup_name("input_mutex");
	...
	list_for_each_entry(dev, input_dev_list, node) {
		if (test_bit(EV_ABS, dev->evbit) &&
            ...
            pr_info("[paradise] Name: %s, Bus: %d Vendor: %d Product: %d Version: %d\n",
					dev->name,
...
int input_event_cache(unsigned int type, unsigned int code, int value, int lock) {
	if (!my_input_handle_event) {
		pr_err("[paradise] Failed to find input_handle_event\n");
		return -EINVAL;
	}
	...
	struct paradise_touch_event* event = &pool->events[pool->size++];
	event->type = type;
	event->code = code;
	event->value = value;
	if (lock)
...
static void handle_cache_events(struct input_dev* dev) {
	...
	for (int i = 0; i < pool->size; ++i) {
		struct paradise_touch_event event = pool->events[i];
...
//	if (type == EV_ABS) {
//		pr_info("[paradise] input_event(%u, %u, %d)", type, code, value);
//	}
...
//	if (type == EV_ABS) {
//		pr_info("[paradise] input_inject_event(%u, %u, %d)", type, code, value);
//	}
...
int init_input_dev(void) {
	...
	pr_info("[paradise] input_event_kp: %d\n", ret);
	if (ret) {
		return ret;
	}
	
	ret = register_kprobe(&input_inject_event_kp);
	pr_info("[paradise] input_inject_event_kp: %d\n", ret);
	if (ret) {
...
*/
	...
	if (!my_input_handle_event) {
		pr_err("[paradise] failed to find my_input_handle_event\n");
		return -1;
	}
