...
uintptr_t get_module_base(pid_t pid, char *name, int vm_flag) {
	...
	if (name_len == 0) {
		pr_err("[paradise] module name is empty\n");
		return 0;
	}

    pid_struct = find_get_pid(pid);
    if (!pid_struct) {
		pr_err("[paradise] failed to find pid_struct\n");
        return 0;
    }
    ...
    put_pid(pid_struct);
    if (!task) {
		pr_err("[paradise] failed to get task from pid_struct\n");
        return 0;
    }
    ...
    if (!mm) {
		pr_err("[paradise] failed to get mm from task\n");
        return 0;
    }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0) && defined(PARADISE_0X202501232139)
    spinlock_t *ptlp;
#endif
...
#endif
	if (!ptep) {
		pr_err("[paradise] failed to get ptep from 0x%lx\n", va);
		return 0;
	}

    if (!pte_present(*ptep)) {
		pr_err("[paradise] pte not present\n");
        return 0;
    }
...
#error unsupported kernel version：__pte_to_phys or pte_pfn
    ...
    if (page_addr == 0) { // why?
		pr_err("[paradise] page_addr is 0\n");
        return 0;
    }
...
int read_process_memory_ioremap(pid_t pid, void __user*addr, void __user*dest, size_t size) {
	...
	if (!addr) {
		pr_err("[paradise] addr is null: %s\n", __func__);
		return -EINVAL;
	}

	if (!access_ok(dest, size)) {
		pr_err("[paradise] access_ok failed: %s\n", __func__);
		return -EACCES;
	}

    ret = pid_vaddr_to_phy(pid, addr, &phy_addr);
	if (ret) {
		pr_err("[paradise] pid_vaddr_to_phy failed: %s\n", __func__);
		return ret;
	}

	if (!pfn_valid(__phys_to_pfn(phy_addr))) {
		pr_err("[paradise] pfn_valid failed: %s\n", __func__);
		return -EFAULT;
	}

	if (!IS_VALID_PHYS_ADDR_RANGE(phy_addr, size)) {
		pr_err("[paradise] IS_VALID_PHYS_ADDR_RANGE failed: %s, ptr = %p, size = %zu\n", __func__, (void *)phy_addr, size);
		return -EFAULT;
	}
    ...
    if (phy_addr){
		...
		pr_err("[paradise] phy_addr is 0, %p, %s\n", (void *)phy_addr, __func__);
		ret = -EFAULT;
	}

	//pr_info("[paradise] read_process_memory_ioremap: %s, ret = %d\n", __func__, ret);
    return ret;
}
...
int access_process_vm_by_pid(pid_t from, void __user*from_addr, pid_t to, void __user*to_addr, size_t size) {
	...
	if (!pid_struct) {
		pr_err("[paradise] failed to find pid_struct(from): %s\n", __func__);
		return -ESRCH;
	}
	...
	put_pid(pid_struct);
	if(!task) {
		pr_err("[paradise] failed to get task from pid_struct(from): %s\n", __func__);
		return -ESRCH;
	}
	...
	if (!pid_struct) {
		pr_err("[paradise] failed to find pid_struct(to): %s\n", __func__);
		vfree(buf);
		return -ESRCH;
	}
	...
	put_pid(pid_struct);
	if(!task) {
		pr_err("[paradise] failed to get task from pid_struct(to): %s\n", __func__);
		vfree(buf);
		return -ESRCH;
	}
