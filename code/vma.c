...
int remap_process_memory(struct vm_area_struct *vma, unsigned long pfn, size_t size) {
	...
	if (!my_remap_pfn_range) {
		my_remap_pfn_range = (void *) paradise_kallsyms_lookup_name("remap_pfn_range");
		if (!my_remap_pfn_range) {
			pr_err("[paradise] failed to find io_remap_pfn_range: %s\n", __func__);
			return -ENOSYS;
		}
	}
...
static int paradise_mremap(const struct vm_special_mapping *sm,
			 struct vm_area_struct *new_vma) {
	return 0;
}

static vm_fault_t paradise_fault(const struct vm_special_mapping *sm,
	...
	if (!pfn) {
		pr_err("[paradise] failed to find pfn: addr = 0x%lx\n", vma->vm_start);
...
static struct vm_special_mapping aarch64_paradise_map __ro_after_init = {
	.name	= "",
	.mremap = paradise_mremap,
	.fault = paradise_fault
};
...
int alloc_process_special_memory_mm(struct mm_struct* mm, unsigned long addr, size_t size, int writable) {
	...
	if (my_install_special_mapping == NULL) {
		my_install_special_mapping = (void*) paradise_kallsyms_lookup_name("_install_special_mapping");
		if (!my_install_special_mapping) {
			pr_err("[paradise] failed to find install_special_mapping: %s\n", __func__);
			return -ENOSYS;
		}
	}
	...
	ret = my_install_special_mapping(mm, addr, size, flags, &aarch64_paradise_map);
