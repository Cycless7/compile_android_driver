...
pte_t *page_from_virt_kernel(unsigned long addr) {
    ...
    if (!init_mm_ptr) {
        init_mm_ptr = (struct mm_struct *) paradise_kallsyms_lookup_name("init_mm");
    }
    ...
    //pr_debug("[paradise] page_from_virt succes, virt (0x%lx), ptep @ %lx", (uintptr_t) addr, (uintptr_t) ptep);
    return ptep;
}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0) && defined(PARADISE_0X202501232117)
                                 ...
                                 pte_t *ptep, pte_t pte)
{
    ...
    if (__sync_icache_dcache == NULL) {
        __sync_icache_dcache = (f__sync_icache_dcache) paradise_kallsyms_lookup_name("__sync_icache_dcache");
    }
...
#define pte_user_exec(pte)	(!(pte_val(pte) & PTE_UXN))
	...
	if (__sync_icache_dcache == NULL) {
		pr_warn("[paradise] symbol `__sync_icache_dcache` not found\n");
	} else {
    ...
    if (system_supports_mte() && pte_access_permitted(pte, false) &&
        ...
        if (mte_sync_tags == NULL) {
            mte_sync_tags = (f_mte_sync_tags) paradise_kallsyms_lookup_name("mte_sync_tags");
        }
        if (mte_sync_tags == NULL) {
            pr_err("[paradise] symbol `mte_sync_tags` not found\n");
            return -2;
        }
        mte_sync_tags(pte, 1);
    }
...
int protect_rodata_memory(unsigned nr) {
    ...
    addr = (uintptr_t) ((uintptr_t) paradise_find_syscall_table() + nr & PAGE_MASK);
    ptep = page_from_virt_kernel(addr);

    if (!pte_valid(READ_ONCE(*ptep))) { // arm64
        printk(KERN_INFO "[paradise] failed to get ptep from 0x%lx\n", addr);
        return -2;
    }
    pte = READ_ONCE(*ptep);
