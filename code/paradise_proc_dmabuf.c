...
struct paradise_dmabuf_private {
    struct sg_table* sgt;
};

/**
 * paradise_dmabuf_map_dma_buf - Map DMA buffer for device access
 ...
 */
static struct sg_table* paradise_dmabuf_map_dma_buf(struct dma_buf_attachment* attachment, enum dma_data_direction dir) {
    struct paradise_dmabuf_private* priv = attachment->dmabuf->priv;
    return priv->sgt;
}

/**
 * paradise_dmabuf_unmap_dma_buf - Unmap DMA buffer after device access
 * @attachment: DMA buffer attachment
 ...
 */
static void paradise_dmabuf_unmap_dma_buf(struct dma_buf_attachment* attachment, struct sg_table* sgt,
                                      enum dma_data_direction dir) {}

/**
 * paradise_dmabuf_release - Release DMA buffer resources
 ...
 */
static void paradise_dmabuf_release(struct dma_buf* dmabuf) {
    struct paradise_dmabuf_private* priv = dmabuf->priv;
    if (!priv) {
        return;
    }

    paradise_info("releasing dmabuf private data\n");
    if (priv->sgt) {
...
/**
 * paradise_dmabuf_begin_cpu_access - Begin CPU access to DMA buffer
 * @dmabuf: DMA buffer
 * @dir: DMA data direction
 *
 ...
 * Return: 0 on success
 */
static int paradise_dmabuf_begin_cpu_access(struct dma_buf* dmabuf, enum dma_data_direction dir) { return 0; }

/**
 * paradise_dmabuf_end_cpu_access - End CPU access to DMA buffer
 * @dmabuf: DMA buffer
 * @dir: DMA data direction
 *
 ...
 * Return: 0 on success
 */
static int paradise_dmabuf_end_cpu_access(struct dma_buf* dmabuf, enum dma_data_direction dir) { return 0; }

/**
 * paradise_dmabuf_mmap - Map DMA buffer into userspace
 * @dmabuf: DMA buffer
 ...
 */
static int paradise_dmabuf_mmap(struct dma_buf* dmabuf, struct vm_area_struct* vma) {
    struct paradise_dmabuf_private* priv = dmabuf->priv;
    struct page* page;
    unsigned long pfn;
    ...
    if (priv->sgt->nents != 1) {
        paradise_err("invalid number of sg entries: %d\n", priv->sgt->nents);
        return -EINVAL;
    }

    page = sg_page(priv->sgt->sgl);
    if (!page) {
        paradise_err("failed to get page from sg_table\n");
        return -EINVAL;
    }
    ...
    paradise_debug("remapping PFN %lx to vma start %lx\n", pfn, vma->vm_start);
 ...
 */
static const struct dma_buf_ops paradise_dmabuf_ops = {
    .map_dma_buf = paradise_dmabuf_map_dma_buf,
    .unmap_dma_buf = paradise_dmabuf_unmap_dma_buf,
    .release = paradise_dmabuf_release,
    .begin_cpu_access = paradise_dmabuf_begin_cpu_access,
    .end_cpu_access = paradise_dmabuf_end_cpu_access,
    .mmap = paradise_dmabuf_mmap,
};

/**
 * do_create_proc_dma_buf - Create a DMA buffer from a process virtual address
 * @sock: Socket for session management
 * @arg: User-space pointer to paradise_dma_buf_create_cmd
 *
...
int do_create_proc_dma_buf(struct socket* sock, void* arg) {
    struct paradise_dma_buf_create_cmd cmd;
    struct sg_table* sgt;
    struct paradise_dmabuf_private* priv;
    ...
    exp_info.ops = &paradise_dmabuf_ops;
