...
static int paradise_release(struct socket *sock) {
	...
	struct paradise_sock *os = (struct paradise_sock *) ((char *) sock->sk + sizeof(struct sock));
	...
	//pr_info("[paradise] PARADISE socket released!\n");
	return 0;
}

static __poll_t paradise_poll(struct file *file, struct socket *sock,
						 struct poll_table_struct *wait) {
	return 0;
}

static int paradise_setsockopt(struct socket *sock, int level, int optname,
...
__always_inline int paradise_get_process_pid(int len, char __user *process_name_user) {
	int err;
	pid_t pid;
	char* process_name;
...
__always_inline int paradise_get_process_module_base(int len, pid_t pid, char __user *module_name_user, int flag) {
	int err;
	char* module_name;
 ...
 */
static int paradise_getsockopt(struct socket *sock, int level, int optname,
						  char __user *optval, int __user *optlen)
{
	struct sock* sk;
	struct paradise_sock* os;
	int len, alive, ret;
	unsigned long pfn;
	...
	if (!sk)
		return -EINVAL;
	os = ((struct paradise_sock*)((char *) sock->sk + sizeof(struct sock)));

	pr_debug("[paradise] getsockopt: %d\n", optname);
	switch (optname) {
		case REQ_GET_PROCESS_PID: {
			ret = paradise_get_process_pid(level, optval);
			if (ret) {
				pr_err("[paradise] paradise_get_process_pid failed: %d\n", ret);
			}
			break;
		}
		...
		case REQ_ATTACH_PROCESS: {
			...
			os->pid = level;
			pr_info("[paradise] attached process: %d\n", level);
			ret = 0;
			break;
		}
	...
	switch (optname) {
		case REQ_GET_PROCESS_MODULE_BASE: {
			...
			ret = paradise_get_process_module_base(len, os->pid, optval, level);
			break;
		}
		case REQ_READ_PROCESS_MEMORY_IOREMAP: {
			if((ret = read_process_memory_ioremap(os->pid, (void *) optval, (void *) optlen, level))) {
				pr_debug("[paradise] read_process_memory_ioremap failed: %d\n", ret);
			}
			break;
		}
...
int paradise_mmap(struct file *file, struct socket *sock,
	...
	if (!ret) {
		pr_err("[paradise] remap_process_memory failed: %d\n", ret);
	}
	return ret;
}

int paradise_ioctl(struct socket * sock, unsigned int cmd, unsigned long arg) {
	...
	if (cmd == CMD_TOUCH_CLICK_DOWN) {
		...
		if (pool->size >= MAX_EVENTS) {
			pr_warn("[paradise] event pool is full!\n");
			pool->size = 0;
		}
		...
		if (copy_to_user(event_user, &event, sizeof(struct touch_event_base))) {
			pr_err("[paradise] copy_to_user failed: %s\n", __func__);
			return -EACCES;
		}
	...
	if (cmd == CMD_COPY_PROCESS) {
		...
		const struct paradise_sock *os = (struct paradise_sock *) ((char *) sock->sk + sizeof(struct sock));
		if (os->pid == 0) {
			return -ESRCH;
		}
		...
		if (my_copy_process == NULL) {
			my_copy_process = (void*) paradise_kallsyms_lookup_name("copy_process");
		}

		if (my_copy_process == NULL) {
			pr_err("[paradise] copy_process not found!\n");
			return -EFAULT;
		}
		...
		if (!new_task) {
			pr_err("[paradise] copy_process failed!\n");
			return -EFAULT;
		}
	...
	if (cmd == CMD_PROCESS_MALLOC) {
		...
		if (os->cached_count >= MAX_CACHE_KERNEL_ADDRESS_COUNT) {
			pr_err("[paradise] cached_addr_array is full!\n");
			return -ENOMEM;
		}
		...
		struct pid *pid_struct = find_get_pid(os->pid);
		if (!pid_struct) {
			pr_err("[paradise] failed to find pid_struct: %s\n", __func__);
			return -ESRCH;
		}
		...
		put_pid(pid_struct);
		if(!task) {
			pr_err("[paradise] failed to get task from pid_struct: %s\n", __func__);
			return -ESRCH;
		}
		...
		if (!mm) {
			pr_err("[paradise] failed to get mm from task: %s\n", __func__);
			return -ESRCH;
		}
		...
		if (addr == 0) {
			...
			mmput(mm);
			atomic_set(&os->remap_in_progress, 0);
			pr_err("[paradise] get_unmapped_area_mm failed: %s\n", __func__);
			return -ENOMEM;
		}
		...
		if (alloc_process_special_memory_mm(mm, addr, PAGE_SIZE, writable)) {
			...
			mmput(mm);
			atomic_set(&os->remap_in_progress, 0);
			pr_err("[paradise] alloc_process_special_memory_mm failed: %s\n", __func__);
			return -ENOMEM;
		}
		...
		if (!kaddr) {
			pr_err("[paradise] kmalloc failed!: %s\n", __func__);
			atomic_set(&os->remap_in_progress, 0);
			return -ENOMEM;
		}
		...
		pr_info("[paradise] malloced kernel address: 0x%lx, pfn: 0x%lx, magic: 0x%lx\n", kaddr, pfn, *(unsigned long*) kaddr);
		return -2033;
	}
	...
	if (cmd == CMD_HIDE_VMA) {
		...
		if (copy_from_user(&args, (struct hide_vma_args __user*) arg, sizeof(struct hide_vma_args))) {
			pr_err("[paradise] copy_from_user failed: %s\n", __func__);
			return -EACCES;
		}
		...
		if (args.mode == HIDE_X) {
			...
			pr_warn("[paradise] hide mode not supported!\n");
			return -ENOSYS;
		}
...
int paradise_sendmsg(struct socket *sock, struct msghdr *m,
                size_t total_len
) {
	return 0;
}

static struct proto paradise_proto = {
	.name = "NFC_LLCP",
	.owner = THIS_MODULE,
	.obj_size = sizeof(struct sock) + sizeof(struct paradise_sock),
};

static struct proto_ops paradise_proto_ops = {
	.family = PF_DECnet,
	.owner = THIS_MODULE,
...
static int paradise_create(struct net *net, struct socket *sock, int protocol,
					  int kern)
{
	uid_t caller_uid;
	struct sock *sk;
	...
	if (caller_uid != 0) {
		pr_warn("[paradise] Only root can create PARADISE socket!\n");
		return -EAFNOSUPPORT;
	}

	if (sock->type != SOCK_RAW) {
		//pr_warn("[paradise] a PARADISE socker must be SOCK_RAW!\n");
		return -ENOKEY;
	}

	sock->state = SS_UNCONNECTED;

	sk = sk_alloc(net, PF_INET, GFP_KERNEL, &paradise_proto, kern);
	if (!sk) {
		pr_warn("[paradise] sk_alloc failed!\n");
		return -ENOBUFS;
	}

	os = (struct paradise_sock*)((char *) sk + sizeof(struct sock));

	paradise_proto_ops.family = free_family;
	sock->ops = &paradise_proto_ops;
	sock_init_data(sock, sk);

	// Initialize the paradise_sock
	os->pid = 0;
	os->pfn = 0;
	atomic_set(&os->remap_in_progress, 0);
	os->cached_count = 0;
...
static struct net_proto_family paradise_family_ops = {
	.family = PF_DECnet,
	.create = paradise_create,
	.owner	= THIS_MODULE,
};
...
static int register_free_family(void) {
	...
	for(family = free_family; family < NPROTO; family++) {
		...
		if (err)
			...
			free_family = family;
			pr_info("[paradise] Find free proto_family: %d\n", free_family);
			return 0;
		}
	}

	pr_err("[paradise] Can't find any free proto_family!\n");
	return err;
}
...
int init_server(void) {
	...
	err = proto_register(&paradise_proto, 1);
	if (err)
		goto out;
